...
| Item | Reason |
|---|---|
| Megatron driver (`rl-workflow/run_megatron_tp_live_fullstep.sh`) and `run_megatron_fullstep.sh` | Decision 10 (FSDP2 only) — **reversed 2026-09-16: the Megatron TP1 driver is being restored as `run_megatron_fullstep.sh` and becomes what `full_step.sh` execs; see PLAN.md decision 10.** It is not a near-copy of the FSDP driver: Megatron LoRA target names (`linear_qkv`, `linear_fc1`, ... instead of the HF names in the overlays), mbridge / recompute / transformer-config blocks, the `fla` / `mamba_ssm` / `causal_conv1d` / TE shims of the dirty environment, and no way to smoke it in the clean environment. C9 keeps `convert_megatron_to_hf_target_modules` so a future port can derive the target lists from the same overlays. |
